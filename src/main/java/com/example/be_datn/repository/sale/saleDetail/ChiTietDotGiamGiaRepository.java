...
public interface ChiTietDotGiamGiaRepository extends JpaRepository<ChiTietDotGiamGia, Integer> {
    ...
    @Query("SELECT c FROM ChiTietDotGiamGia c WHERE c.idChiTietSanPham.id = :chiTietSanPhamId AND c.deleted = false AND c.idDotGiamGia.trangThai = false and c.idDotGiamGia.trangThai = false")
    Optional<ChiTietDotGiamGia> findByChiTietSanPhamIdAndActive(Integer chiTietSanPhamId);

    @Query("SELECT c FROM ChiTietDotGiamGia c WHERE c.idChiTietSanPham.id = :chiTietSanPhamId AND c.deleted = :deleted AND c.idDotGiamGia.trangThai = false and c.idDotGiamGia.trangThai = false")
