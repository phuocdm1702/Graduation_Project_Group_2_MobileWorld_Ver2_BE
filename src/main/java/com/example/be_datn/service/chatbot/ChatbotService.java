...
//                .stream()
//                .filter(sanPham -> sanPham.getTenSanPham().toLowerCase().contains(request.getMessage().toLowerCase()) ||
//                        (sanPham.getMa() != null && sanPham.getMa().toLowerCase().contains(request.getMessage().toLowerCase())))
//                .toList();
//
//        String productInfo;
//        if (sanPhams.isEmpty()) {
...
//                        "Tên: %s, Mã: %s, Màu: %s, RAM: %s, ROM: %s, Giá: %s, Số lượng: %d",
...
//                %s
//                Nếu không tìm thấy sản phẩm, hãy gợi ý khách hàng liên hệ qua hotline hoặc đến cửa hàng.
...
//    }
    ...
    @Value("${grok.api.key}") // Thay bằng grok.api.key
    private String API_KEY;

    private final String API_URL = "https://api.x.ai/v1/chat/completions"; // Thay bằng endpoint thực tế từ x.ai/api
    private final ObjectMapper objectMapper = new ObjectMapper();
    ...
    public ChatResponse getChatbotResponse(ChatRequest request) throws Exception {
        ...
        // Tạo payload JSON sử dụng ObjectMapper
        ObjectNode requestBody = objectMapper.createObjectNode();
        requestBody.put("model", "grok-3"); // Thay bằng tên mô hình thực tế của Grok
        ...
        try (Response response = client.newCall(apiRequest).execute()) {
            if (!response.isSuccessful()) {
                ...
                if (response.code() == 429) {
                    throw new Exception("Quota exceeded. Please check your Grok plan or try a different approach.");
                } else if (response.code() == 401) {
                    throw new Exception("Invalid API key. Please update your API key at https://x.ai/api.");
                }
                throw new Exception("Error calling Grok API: " + response + ", Details: " + errorBody);
            }
